...
public class DES {
    ...
    public static long encryptBlock(long m, /* 64 bits */ long key) {
        ...
        // apply the final permutation and return
        return FP(rl);
    }
     ...
     */
    public static byte[] encryptCBC(byte[] message, long key) {
        byte[] ciphertext = new byte[message.length];
        ...
        for (int i = 0; i < message.length; i += 8) {
            ...
            long cipherBlock = encryptBlock(messageBlock ^ previousCipherBlock, key);
     ...
     */
    public static byte[] decryptCBC(byte[] ciphertext, long key) {
        byte[] message = new byte[ciphertext.length];
        long previousCipherBlock = IV;
        ...
        for (int i = 0; i < ciphertext.length; i += 8) {
            ...
            // First previousCiphertext = Initial Vector (IV)
            long messageBlock = decryptBlock(cipherBlock, key);
